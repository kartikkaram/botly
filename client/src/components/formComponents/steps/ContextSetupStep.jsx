...
<motion.div 
  className="bg-black p-6 rounded-xl shadow-md border border-gray-800"
...
>
  ...
  <p className="text-gray-400 mb-4">What can your bot do? Add specific capabilities.</p>
  ...
  <div className="flex flex-col sm:flex-row gap-2 mb-4">
    <input
      ...
      className="flex-1 px-4 py-3 border border-gray-700 bg-gray-900 text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-white transition duration-200 placeholder:text-gray-500"
      onKeyDown={(e) => e.key === 'Enter' && addCapability()}
    />
    <motion.button
      onClick={addCapability}
      className="px-6 py-3 bg-white text-black rounded-xl hover:bg-gray-200 transition duration-200 flex items-center justify-center"
  ...
  <AnimatePresence>
    {formData.capabilities.map((capability, index) => (
      <motion.div
        key={index}
        className="flex items-center justify-between bg-gray-800 p-3 rounded-lg mb-2 shadow-sm border border-gray-700"
...
>
  <h3 className="text-xl font-semibold text-white mb-4">Restricted Topics</h3>
  <p className="text-gray-400 mb-4">Topics your bot should avoid discussing.</p>
  ...
  <div className="flex flex-col sm:flex-row gap-2 mb-4">
    <input
      ...
      className="flex-1 px-4 py-3 border border-gray-700 bg-gray-900 text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 transition duration-200 placeholder:text-gray-500"
      onKeyDown={(e) => e.key === 'Enter' && addRestrictedTopic()}
    />
    <motion.button
      onClick={addRestrictedTopic}
      className="px-6 py-3 bg-red-500 text-white rounded-xl hover:bg-red-600 transition duration-200 flex items-center justify-center"
      whileHover={{ scale: 1.05 }}
      whileTap={{ scale: 0.95 }}
    >
      <Plus className="w-5 h-5" />
...
>
  <h3 className="text-xl font-semibold text-zinc-200">Context Input</h3>
    ...
    {contextInputTypes.map((type) => (
      ...
      >
        <type.icon className={`w-6 h-6 mb-2 ${
          formData.contextInputType === type.id ? 'text-blue-400' : 'text-zinc-400'
        }`} />
        <h4 className="font-semibold text-zinc-200">{type.label}</h4>
  ...
  <AnimatePresence mode="wait">
    {formData.contextInputType === 'manual' && (
      <motion.div
        ...
        exit={{ opacity: 0, y: -20 }}
      >
        <h4 className="font-semibold text-zinc-200">Add FAQ Pairs</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-2">Question/Input</label>
            <textarea
              value={newFaqInput}
              onChange={(e) => setNewFaqInput(e.target.value)}
              placeholder="What can you help me with?"
              className={`w-full px-4 py-3 rounded-xl text-zinc-100 placeholder:text-zinc-400 backdrop-blur-sm ${
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-2">Answer/Output</label>
            <textarea
              value={newFaqOutput}
        ...
        <motion.button
          onClick={addFaqPair}
          className="w-full py-3 bg-green-500 text-white rounded-xl hover:bg-green-600 transition duration-200 flex items-center justify-center"
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
        >
    ...
    {formData.contextInputType === 'json' && (
      ...
      >
        ...
        <textarea
          ...
          className={`w-full px-4 py-3 rounded-xl font-mono text-zinc-100 placeholder:text-zinc-400 backdrop-blur-sm ${
  ...
  >
    ...
    <motion.input
      ...
      className={`w-full px-4 py-3 rounded-xl text-zinc-100 placeholder:text-zinc-400 backdrop-blur-sm transition-all duration-200 ${
